...
function StatCard({ title, value, icon: Icon, color }) {
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-5 border flex justify-between items-center">
      <div>
...
export default function Inventory() {
  ...
  return (
    <div className="space-y-6 px-3 sm:px-0 pb-10">

      {/* STATS */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
        <StatCard title="Total Items" value={stats.total} icon={Package} color="bg-teal-100 text-teal-600" />
        <StatCard title="Low Stock" value={stats.low} icon={AlertTriangle} color="bg-yellow-100 text-yellow-600" />
        <StatCard title="Out of Stock" value={stats.out} icon={XCircle} color="bg-red-100 text-red-500" />
        <StatCard title="Expiring Soon" value={stats.expiringSoon} icon={Clock} color="bg-blue-100 text-blue-600" />
        <StatCard title="Expired" value={stats.expired} icon={CalendarX2} color="bg-red-100 text-red-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border p-5">
          ...
          <div className="flex gap-2 h-20 sm:h-24">
            <div className="bg-teal-500 rounded-xl" style={{ width: `${categoryBreakdown.MEDICATIONS}%` }} />
            <div className="bg-teal-300 rounded-xl" style={{ width: `${categoryBreakdown.CONSUMABLES}%` }} />
            <div className="bg-teal-200 rounded-xl" style={{ width: `${categoryBreakdown.LABORATORY}%` }} />
            <div className="bg-gray-200 rounded-xl" style={{ width: `${categoryBreakdown.OTHER}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <InventoryTable
            search={search}
            status={status}
            onMutation={() => setRefreshKey((v) => v + 1)}
          ...
          <div className="space-y-2 text-sm">
            {activities.length === 0 ? (
              ...
              activities.map((a, i) => {
                ...
                return (
                  <div key={a.id || i} className="flex gap-3 p-2 hover:bg-gray-50 rounded-lg">
