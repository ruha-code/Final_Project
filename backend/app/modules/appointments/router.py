...
@router.post("", response_model=AppointmentDetailResponse, status_code=201, summary="Book an appointment")
async def book_appointment(
    ...
    if not (schedule.start_time <= appt_time.time() < schedule.end_time):
        raise ValidationException(f"Outside working hours ({schedule.start_time}–{schedule.end_time})")
 
    if await _check_doctor_conflict(doctor.id, appt_time, doctor.consultation_duration_minutes, db):
        raise ConflictException("Doctor is already booked at this time")
 
    if await _check_patient_conflict(patient.id, appt_time, doctor.consultation_duration_minutes, db):
    ...
    await log(db, current_user.id, Actions.BOOK_APPOINTMENT, entity_type="Appointment", entity_id=appointment.id)
    await event_bus.publish("appointment_created", {"appointment_id": appointment.id})
    return _build_detail(appointment)
 
 
@router.get("/my", response_model=list[AppointmentDetailResponse], summary="Get my appointments")
async def get_my_appointments(
    ...
    if current_user.role == "PATIENT":
        ...
        query = select(Appointment).options(*_load_opts()).where(Appointment.patient_id == patient.id)
    else:
        result = await db.execute(select(Doctor).where(Doctor.user_id == current_user.id))
        doctor = result.scalar_one_or_none()
        if not doctor:
            raise NotFoundException("Doctor profile not found")
        query = select(Appointment).options(*_load_opts()).where(Appointment.doctor_id == doctor.id)
 
    if status:
        query = query.where(Appointment.status == status.upper())
    if from_date:
        query = query.where(Appointment.appointment_time >= datetime.combine(from_date, time.min))
    if to_date:
        query = query.where(Appointment.appointment_time <= datetime.combine(to_date, time.max))
...
@router.put("/{appointment_id}/complete", response_model=AppointmentResponse, summary="Complete an appointment (Doctor)")
...
@router.get("/admin/all", summary="List all appointments (Admin)", dependencies=[Depends(RoleChecker(["ADMIN"]))])
...
@router.put("/{appointment_id}/admin-cancel", summary="Admin cancel appointment", dependencies=[Depends(RoleChecker(["ADMIN"]))])
